...
# Date: 2015-10-30
# Author: Gabriel Chan
# Comment: clone from iibdeploy.sh to state script
...
_execSTATE() {
  PADTEXT='    STATE> '
  _PAD=$PADTEXT notify "${_BAR_STATE}"
  OUTPUT="$(${_BAR_STATE} 2>&1 )"
  _RC=$?
...
_RC=$?
if [ "$_RC" -ne 0 ]; then
  warn "iibstate.sh: Failed to source ../resources/$USER-env.sh"
fi
...
_RC=$?
if [ "$_RC" -ne 0 ]; then
  warn "iibstate.sh: Failed to source../resources/iibdeploy-${_NODE_}-env.sh"
fi
sourceConfig ../resources/${_HOSTNAME}-env.sh
_RC=$?
if [ "$_RC" -ne 0 ]; then
  debug "iibstate.sh: Failed to source ../resources/${_HOSTNAME}-env.sh"
fi
...
if [ "$_HOSTNAME" = "localhost" ]; then
  sourceConfig ../resources/iibstate-localhost-env.sh
  _RC=$?
  if [ "$_RC" -ne 0 ]; then
    warn "iibstate.sh: failed to source ../resources/iibstate-localhost-env.sh"
  fi
  REMOVE=REMOVE-localhost.sh
else
  sourceConfig ../resources/iibstate-env.sh
  _RC=$?
  if [ "$_RC" -ne 0 ]; then
    warn "iibstate.sh: failed to source ../resources/iibstate-env.sh"
  fi
  REMOVE=REMOVE.sh
fi

debug "iibstate.sh: calling _execSTATE"
_execSTATE
_RC=$?
