@REM File: build\MQSI_RESTART.bat
@REM Date: 6-Nov 2014
@REM Version: 0.1.1
...
IF "x%_MODE%" == "x/stop" (
 IF !_FLAG! EQU 1 (
  ...
  pause
  ECHO _SHELL_BRKADM: %_SHELL_BRKADM% ^"mqsistop %_NODE%^"
  %_SHELL_BRKADM% "mqsistop %_NODE%"
...
IF "x%_MODE%" == "x/start" (
 IF !_FLAG! EQU 1 (
  ...
  ECHO _SHELL_BRKADM: %_SHELL_BRKADM% ^"mqsistart %_NODE%^"
  %_SHELL_BRKADM% "mqsistart %_NODE%"
 )
)