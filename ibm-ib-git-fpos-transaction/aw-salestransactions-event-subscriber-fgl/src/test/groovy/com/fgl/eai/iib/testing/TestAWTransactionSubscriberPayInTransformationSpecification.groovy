...
import com.fgl.eai.iib.testing.scenarios.AWTransactionSubscriberPayInScenarios
 ...
 */
class TestAWTransactionSubscriberPayInTransformationSpecification extends BaseSpecification {
    ...
    static def scenarios = new AWTransactionSubscriberPayInScenarios()

    @Unroll
    def "AW Transaction Subscriber Pay In Transaction Validate XML Node Values"() {
        setup:
            XmlParser p = new XmlParser()
            p.setFeature("http://xml.org/sax/features/namespaces", false)
            groovy.util.Node body = p.parseText stubs.getXmlPayIn()
