...
import com.fgl.eai.iib.testing.scenarios.*
 ...
 */
class TestAWTransactionSubscriberTaxTransformationSpecification extends BaseSpecification {
    ...
    static def scenarios = new AWTransactionSubscriberTaxCodeScenarios()

    @Unroll
    def "AW Transaction Subscriber Taxes Validate XML Node Values"() {
        setup:
            XmlParser p = new XmlParser()
            p.setFeature("http://xml.org/sax/features/namespaces", false)
            groovy.util.Node body = p.parseText stubs.getXml()
        ...
        where:
			...
			// basic tests for tax code translation
            ASSERT_TYPE_XML            | 0
            ASSERT_TYPE_XML            | 1
            ASSERT_TYPE_XML            | 2
            ASSERT_TYPE_XML            | 3
