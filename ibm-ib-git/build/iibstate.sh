...
printUsage() {
 cat << EOF
iibstate <project> <collection> [<environment>] [<instance>] [<execgroup>]
Must use specific project name (no wildcards)
For example: iibstate doc-transfer-pmm-publisher caldc PP
EOF
 RC=1
 exit $RC
}
...
doFinish() {
    trace "iibstate.sh: doFinish($*)"
  if [ "$EC" -eq 0 ]; then
    _message="project '${_PROJECT}' for collection '${_COLLECTION}' was checked [PASS]"
    inf "$_message"
  else
    _message="project '${_PROJECT}' for collection '${_COLLECTION}' was not checked [FAIL]"
    warn "$_message"
  fi
...
doState() {
  trace "iibstate.sh: doState($*)"
  _a=$1
  _message="project '$_a' for collection '${_COLLECTION}' processing.."
  _b=1
  inf "$_message"
  debug "iibstate.sh: calling STATE.sh \"$_COLLECTION\" \"$_a\""
  . $CWD/STATE.sh "${_COLLECTION}" "${_a}"
  EC=$?
  if [ "$EC" -eq 0 ]; then
...
if [ "${#*}" -gt 0 ]; then
  ...
  COMMAND_ARGS=($*)
  $_globstate
  debug "iibstate.sh: $COUNT_ARGS arguments read"
  debug "iibstate.sh: $*"
fi
...
# override _SERVER from resource file if _EXECGROUP is passed in
...
debug "iibstate.sh: Calling doState $_PROJECT"
doState "$_PROJECT"
