...
import com.fgl.eai.iib.testing.scenarios.AWTransactionSubscriberNoSaleScenarios
 ...
 */
class TestAWTransactionSubscriberNoSaleTransformationSpecification extends BaseSpecification {
    ...
    static def scenarios = new AWTransactionSubscriberNoSaleScenarios()

    @Unroll
    def "AW Transaction Subscriber No Sale Transaction Validate XML Node Values"() {
        setup:
            XmlParser p = new XmlParser()
            p.setFeature("http://xml.org/sax/features/namespaces", false)
            groovy.util.Node body = p.parseText stubs.getXmlNoSale()
        ...
        where:
			...
			// basic tests for no sale code translation
            ASSERT_TYPE_XML            | 0
            ASSERT_TYPE_XML            | 1
            ASSERT_TYPE_XML            | 2
            ASSERT_TYPE_XML            | 3
